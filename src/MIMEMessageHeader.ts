...
export class MIMEMessageHeader {
    fields: HeaderField[] = [
        ...
        },
        {
            ...
            required: true,
            dump: (v: unknown) => {
                return typeof v === 'string' ? '=?utf-8?B?' + toBase64(v) + '?=' : ''
            }
        },
        {
            name: 'MIME-Version',
    ...
    dumpMailboxMulti(v: unknown): string {
        const dump = (item: Mailbox) => item.name.length === 0
            ? item.dump()
            : `=?utf-8?B?${toBase64(item.name)}?= <${item.addr}>`
        return this.isArrayOfMailboxes(v) ? v.map(dump).join(`,\r\n `) : v instanceof Mailbox ? dump(v) : ''
    }
